...
@router.get("/staff_invites", response_class=HTMLResponse, dependencies=[Depends(require_roles("superadmin", "admin", "manager"))])
async def invites_page(request: Request, current_user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse("staff_invites.html", {
        "request": request,
        "user": current_user
    })
